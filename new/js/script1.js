...
function chargerPublicite(){
	var donnees = {"numeroMatricule":numeroMatricule};
	alert(adresseServeur+"/publicite.php");
	$.get(adresseServeur+"/publicite.php",donnees,function(recu){
		$("#zonePublicite").html(recu);
	});
}
...
function creerChampDefaut(){
	var zone = document.getElementById("anciensMessages");
	var eltDiv = document.createElement("div");
	eltDiv.setAttribute("class","unMessage moi");
	var txt = document.createTextNode("Salutt");
	eltDiv.appendChild(txt);
	//$(eltDiv).html("Tres bien");
	zone.appendChild(eltDiv);
	...
	if(numeroCorrespondant != "" && testEnTrainDecrire == false){
		...
		$.post(adresseMessagerie,donnees,function(recu){
			...
			if(recu == "0"){
				...
				var eltDiv = document.createElement("div");
				eltDiv.setAttribute("class","unMessage correspondant");
...
function creerNouveauElementText(){
	...
	zoneDate.setAttribute("class","laDate");
